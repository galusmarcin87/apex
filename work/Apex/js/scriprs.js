const accordionInit = (
  ...
  $header.next('.collapse').removeClass('collapsed').slideToggle();
  ...
  $accordion.find('.collapsed + div').slideUp();
};
...
$(document).ready(function () {
  ...
  $('.Image-slider, .Project__gallery__photo').magnificPopup(
    {
      type: 'image',
      delegate: 'a',
      gallery: {
   ...
   $('.Project__gallery__photo .zoom').magnificPopup({
     type: 'image',
     delegate: 'a',
     gallery: {
       enabled: true,
  ...
  $('body').on('click', '.Gallery__wrapper .item', function (evt) {
    const href = $(this).attr('href');

    $('.Project__photo').attr('src', href).siblings('.zoom').attr('href', href);
    evt.preventDefault();
  });
  ...
  $('.counter [data-to]').viewportChecker({
    callbackFunction: function (elem, action) {
      const countTo = $(elem).attr('data-to');
      $({ countNum: 0 }).animate(
        {
...

        {
          ...
          complete: function () {
            $(elem).text(String(this.countNum).replace('.', ','));
          },
        }
      );
    },
  ...
  $('.Projects__card, .News .Card:not(.Card--single)').on(
    'click',
    function (e) {
      $(this).find('a')[0].click();
    }
  ...
  $('.Menu-top__toggle-btn').on('click', function (e) {
    ...
    $('.Menu-top__list').toggleClass(function () {
      if (!$(this).is(':visible')) {
