...
//        }
    ...
    public function actionSetClientData()
    {

        $user = User::find()->where(['adviser_id' => MgHelpers::getUserModel()->id, 'id' => Yii::$app->request->post('userId')])->one();
        if(!$user){
            $this->throw404();
        }
        if(Yii::$app->request->post('additional_information')){
            $user->additional_information = Yii::$app->request->post('additional_information');
            $user->save();
        }
        if(Yii::$app->request->post('project_id')){
            $user->project_id = Yii::$app->request->post('project_id');
            $user->save();
        }
